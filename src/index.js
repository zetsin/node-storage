...
const createStorage = (dir='storage', opts={}) => {
  ...
  storage.clear = function(all=false) {
  ...
  storage.set = function(key='', value='', opts={}) {
    const filename = name(key)
    const pathname = path.join(dir, filename)
  ...
  storage.cleanup = function(all=false) {
    return all ? (
      fs.remove(dir)
    ) : (
      this.list()
      .then(files => Promise.all(files.map(file => this.remove(file.key))))
    )
    .catch(err => Promise.resolve({}))
  }
  storage.cleanout = function() {
    this.list()
    .then(files => {
      const now = Date.now()
      return Promise.all(files.map(file => {
        if(file.created + file.ttl < now) {
